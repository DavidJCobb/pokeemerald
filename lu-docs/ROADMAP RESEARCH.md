...
Alternatively... I'm curious if it's possible to add some sort of last-second step to the linker, wherein we can extract whatever bytes ended up at specific ROM locations (the ones targeted by cheat codes), and have it duplicate them at some other fixed location. Think of it like creating a control group to which we can compare an experimental group: our code can compare the data at the original ROM locations to the copied data, and if a mismatch is found, then we know a cheat device is shimming those ROM locations. Wouldn't help us at all for RAM, though, but part of what I'm thinking of is that discontiguous sections might be better for RAM as opposed to ROM (where there are lots of large arrays, and if one of them hits a section boundary, then oop, that's it, we just wasted tons of ROM moving that whole array to the next section).
