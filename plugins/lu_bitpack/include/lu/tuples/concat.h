...
namespace lu::tuples {
   namespace impl::_concat {
      ...
      // Join two tuples end-to-end.
      template<class, class> struct two;
      //
      template<typename... A, typename... B>
      struct two<std::tuple<A...>, std::tuple<B...>> {
      ...
      template<class... T> struct arbitrary {
         ...
         // for brevity:
         template<size_t n>
         using nth_type = typename std::tuple_element<n, full_list>::type;
   ...
   template<class... T>
   using concat = impl::_concat::arbitrary<T...>::type;
}
