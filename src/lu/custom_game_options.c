...
void ResetCustomGameOptions(void) {
   ...
   gCustomGameOptions.starter_data.starter_trio[0] = 0;
   gCustomGameOptions.starter_data.starter_trio[1] = 0;
   gCustomGameOptions.starter_data.starter_trio[2] = 0;
...
void BitWriteCustomGameOptions(struct CustomGameOptions* cgo, struct lu_BitstreamState* state) {
   ...
   switch (cgo->starter_mode) {
      case 0:
         lu_BitstreamWrite_u16(state, cgo->starter_data.starter_trio[0], BITS_FOR_POKEMON_SPECIES);
         lu_BitstreamWrite_u16(state, cgo->starter_data.starter_trio[1], BITS_FOR_POKEMON_SPECIES);
         lu_BitstreamWrite_u16(state, cgo->starter_data.starter_trio[2], BITS_FOR_POKEMON_SPECIES);
         break;
      case 1:
         lu_BitstreamWrite_u16 (state, cgo->starter_data.single_species.player_species, BITS_FOR_POKEMON_SPECIES);
         lu_BitstreamWrite_u16 (state, cgo->starter_data.single_species.rival_base_species, BITS_FOR_POKEMON_SPECIES);
         lu_BitstreamWrite_u8  (state, cgo->starter_data.single_species.catch_mode, 1);
         lu_BitstreamWrite_bool(state, cgo->starter_data.single_species.give_new_mon_after_gyms);
   ...
   lu_BitstreamWrite_bool(state, cgo->nuzlocke.encounter_limit.dupes_clause.enabled);
   lu_BitstreamWrite_u8  (state, cgo->nuzlocke.encounter_limit.dupes_clause.shiny_exception, 2);
}

void BitReadCustomGameOptions(struct CustomGameOptions* cgo, struct lu_BitstreamState* state) {
   u16 version;
   ...
   switch (cgo->starter_mode) {
      case 0:
         cgo->starter_data.starter_trio[0] = lu_BitstreamRead_u16(state, BITS_FOR_POKEMON_SPECIES);
         cgo->starter_data.starter_trio[1] = lu_BitstreamRead_u16(state, BITS_FOR_POKEMON_SPECIES);
         cgo->starter_data.starter_trio[2] = lu_BitstreamRead_u16(state, BITS_FOR_POKEMON_SPECIES);
         break;
      case 1:
         cgo->starter_data.single_species.player_species     = lu_BitstreamRead_u16(state, BITS_FOR_POKEMON_SPECIES);
         cgo->starter_data.single_species.rival_base_species = lu_BitstreamRead_u16(state, BITS_FOR_POKEMON_SPECIES);
         cgo->starter_data.single_species.catch_mode = lu_BitstreamRead_u8(state, 1);
         cgo->starter_data.single_species.give_new_mon_after_gyms = lu_BitstreamRead_bool(state);
         #if SINGLE_SPECIES_RUN_SPARE_BITS > 0
            #if SINGLE_SPECIES_RUN_SPARE_BITS <= 8   
   ...
   cgo->nuzlocke.encounter_limit.dupes_clause.enabled         = lu_BitstreamRead_bool(state);
